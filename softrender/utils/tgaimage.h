// Credits to Dmitriy V. Sokolov aka haqreu
...
struct TGAColor {
    ...
    TGAColor operator +(const TGAColor& other) const {
        TGAColor res = *this;
        for (int i=0; i<4; i++) {
			uint8_t clamped = std::max(0, std::min(int(0.5 * (this->bgra[i] + other.bgra[i])), 255));
	        res.bgra[i] = clamped;
        }
        return res;
    }
